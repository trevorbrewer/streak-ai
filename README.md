# STREAK·AI — Beat the Streak Prediction System

![CI Tests](https://github.com/trevorbrewer/streak-ai/actions/workflows/ci.yml/badge.svg)
![Daily Pipeline](https://github.com/YOUR_USERNAME/streak-ai/actions/workflows/daily_picks.yml/badge.svg)
![Python 3.11+](https://img.shields.io/badge/python-3.11+-blue.svg)
![License: MIT](https://img.shields.io/badge/License-MIT-yellow.svg)

> AI-powered MLB hit prediction system for ESPN Beat the Streak.
> Automatically pulls daily schedules, enriches hitter matchups,
> engineers advanced features, and delivers ranked picks via email every morning.

## Status

🚧 Under active development — see the [project roadmap](#roadmap) below.

## Roadmap

| Chunk | Description | Status |
|-------|-------------|--------|
| 1 | Repo setup & project scaffold | ✅ Done |
| 2 | Python environment & config | ✅ Done |
| 3 | Hitter data model & storage | ⬜ |
| 4 | GitHub Actions CI pipeline | ⬜ |
| 5 | MLB Stats API — season stats | ⬜ |
| 6 | Daily schedule & matchup puller | ⬜ |
| 7 | Statcast & advanced metrics | ⬜ |
| 8 | Weather & park factors | ⬜ |
| 9 | Feature engineering pipeline | ⬜ |
| 10 | Claude AI scoring engine | ⬜ |
| 11 | Full pipeline orchestrator | ⬜ |
| 12 | GitHub Actions daily automation | ⬜ |
| 13 | Email report module | ⬜ |
| 14 | Web dashboard (GitHub Pages) | ⬜ |
| 15 | README, docs & portfolio polish | ⬜ |

## Tech Stack

- **Python 3.11** — core language
- **Anthropic Claude** — AI hitter analysis and scoring
- **MLB Stats API** — free schedule, pitcher, and lineup data
- **pybaseball** — Statcast advanced metrics
- **SendGrid** — automated email delivery
- **GitHub Actions** — CI/CD and daily pipeline automation
- **GitHub Pages** — live dashboard

## Setup

*Full setup instructions coming in Chunk 2.*

## License

MIT — see [LICENSE](LICENSE)
